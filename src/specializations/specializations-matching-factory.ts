...
export class SpecializationsMatchingFactory {
    public static async create(): Promise<SpecializationsMatchingEngine> {
        ...
        const synonymsData: WordSynonym[] = [
            ...
            // В справочнике специализация называется «Golang разработчик», а вакансии сплошь и
            // рядом пишут «Go разработчик». Без этой пары движок не сопоставлял такой текст НИ С
            // ЧЕМ (не выдавал неверную специализацию — возвращал null), и вакансия оставалась без
            // ссылки. Отдельного «Go …» в справочнике нет, поэтому пара может только помочь.
            // Вес трогать не нужно: `{ word: 'go', weight: 2.0 }` ниже сам проходит через
            // `normalize()` при сборке карты весов, то есть ключом станет 'golang'.
            { src: 'go', dst: 'golang' },
        ];
